...
import { usePlantData } from "@/components/plantdata";

export default function Monitor() {
  const plants = [{ id: 1, image: "/plant1.jpg" }];
  const [selectedPlant, setSelectedPlant] = useState(plants[0]);
  const { data: sensorData, loading, error } = usePlantData();
          ...
          <CardContent className="p-6">
            <div className="flex gap-6 mt-4">
              ...
              <div className="flex-1">
                <PlantDataChart data={sensorData} />
              </div>
            </div>
          </CardContent>
        </Card>
